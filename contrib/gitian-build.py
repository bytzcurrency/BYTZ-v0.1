...
# Copyright (c) 2019 The Bytz developers
...
def setup():
    ...
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/gitianuser/gitian.sigs-bytz.git', 'gitian.sigs'])
    if not os.path.isdir('bytz-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/gitianuser/bytz-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('bytz'):
        subprocess.check_call(['git', 'clone', 'https://github.com/SlateEntertainmentGroup/bytz.git'])
...
def verify():
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../bytz/contrib/gitian-descriptors/gitian-linux.yml'])
        print('\nVerifying v'+args.version+' Linux\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../bytz/contrib/gitian-descriptors/gitian-linux.yml'])

    if args.windows:
        print('\nVerifying v'+args.version+' Windows\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../bytz/contrib/gitian-descriptors/gitian-win.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed Windows\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../bytz/contrib/gitian-descriptors/gitian-win-signer.yml'])

    if args.macos:
        print('\nVerifying v'+args.version+' MacOS\n')
        subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../bytz/contrib/gitian-descriptors/gitian-osx.yml'])
        if args.sign:
            print('\nVerifying v'+args.version+' Signed MacOS\n')
            subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../bytz/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/SlateEntertainmentGroup/bytz/', help='Specify the URL of the repository. Default is %(default)s')
