...
CzBYTZWallet::CzBYTZWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbytz seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBYTZSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBYTZSeed_deprecated()) {
                    LogPrintf("%s: Updated zBYTZ seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbytz seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbytz wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBYTZWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBYTZCount(nCountLastUsed);
    else if (!walletdb.ReadZBYTZCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBYTZWallet::Lock()
{
    seedMaster = 0;
}

void CzBYTZWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBYTZWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBYTZ(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBYTZWallet::LoadMintPoolFromDB()
{
...
void CzBYTZWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBYTZWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBYTZWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbytzTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBYTZWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBYTZ(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbytzTracker which also adds to database
    pwalletMain->zbytzTracker->Add(dMint, true);
...
void CzBYTZWallet::SeedToZBYTZ(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBYTZWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBYTZWallet::GenerateDeterministicZBYTZ(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBYTZWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBYTZWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
