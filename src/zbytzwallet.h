...
#ifndef BYTZ_ZBYTZWALLET_H
#define BYTZ_ZBYTZWALLET_H
...
public:
    CzBYTZWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBYTZ(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBYTZ(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BYTZ_ZBYTZWALLET_H
