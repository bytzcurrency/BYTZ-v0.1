// Copyright (c) 2017-2018 The BYTZ developers
...
CZBytzStake::CZBytzStake(const libzerocoin::CoinSpend& spend)
{
...
int CZBytzStake::GetChecksumHeightFromMint()
{
...
int CZBytzStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZBytzStake::GetChecksum()
{
    return nChecksum;
}
...
// 100 blocks deep.
CBlockIndex* CZBytzStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZBytzStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZBytzStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZBytzStake::GetUniqueness()
{
...
bool CZBytzStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZBytzStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    for (unsigned int i = 0; i < 3; i++) {
        ...
        if (!pwallet->CreateZBYTZOutPut(libzerocoin::CoinDenomination::ZQ_TEN, out, dMintReward))
            return error("%s: failed to create zBYTZ output", __func__);
...
bool CZBytzStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZBytzStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzBYTZTracker* zbytzTracker = pwallet->zbytzTracker.get();
    CMintMeta meta;
...
//!BYTZ Stake
bool CBytzStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBytzStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CBytzStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBytzStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CBytzStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CBytzStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CBytzStake::GetUniqueness()
{
    //The unique identifier for a BYTZ stake is the outpoint
...
CBlockIndex* CBytzStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
