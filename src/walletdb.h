...
public:
    ...
    bool WriteZBYTZSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBYTZSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBYTZSeed_deprecated(uint256& seed);
    bool EraseZBYTZSeed();
    bool EraseZBYTZSeed_deprecated();

    bool WriteZBYTZCount(const uint32_t& nCount);
    bool ReadZBYTZCount(uint32_t& nCount);
