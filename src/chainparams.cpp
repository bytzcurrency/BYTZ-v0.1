...
public:
    CMainParams()
    {
        ...
        nDefaultPort = 37415;
        bnProofOfWorkLimit = ~uint256(0) >> 20; // BYTZ starting difficulty is 1 / 2^12
        ...
        nMinerThreads = 0;
        nTargetTimespan = 1 * 60; // BYTZ: 1 day
        nTargetSpacing = 1 * 60;  // BYTZ: 1 minute
        nMaturity = 100;
        ...
        nZerocoinRequiredStakeDepth = 200; //The required confirmations for a zbytz to be stakable
...
public:
    CTestNetParams()
    {
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        vSeeds.push_back(CDNSSeedData("192.219.30.30", "192.219.30.30"));   // Bytz seeder located in Dublin
        vSeeds.push_back(CDNSSeedData("192.219.30.29", "192.219.30.29"));   // Bytz seeder located in Dublin
        vSeeds.push_back(CDNSSeedData("204.92.235.7", "204.92.235.7"));     // Bytz seeder located in Montreal
        vSeeds.push_back(CDNSSeedData("198.96.117.222", "198.96.117.222")); // Bytz seeder located in Montreal

        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 66); // Testnet bytz addresses start with 'T'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 9);  // Testnet bytz script addresses start with '4' or '5'
        base58Prefixes[SECRET_KEY] = std::vector<unsigned char>(1, 144);     // Testnet private keys start with 'z' or '2'
        // Testnet bytz BIP32 pubkeys start with 'DRKV'
        base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x3a)(0x80)(0x61)(0xa0).convert_to_container<std::vector<unsigned char> >();
        // Testnet bytz BIP32 prvkeys start with 'DRKP'
...
public:
    CRegTestParams()
    {
        ...
        nToCheckBlockUpgradeMajority = 1000;
        nMinerThreads = 1;
        nTargetTimespan = 24 * 60 * 60; // BYTZ: 1 day
        nTargetSpacing = 1 * 60;        // BYTZ: 1 minutes
