...
#include "ui_zbytzcontroldialog.h"
...
std::set<std::string> ZBytzControlDialog::setSelectedMints;
std::set<CMintMeta> ZBytzControlDialog::setMints;

ZBytzControlDialog::ZBytzControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZBytzControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZBytzControlDialog::~ZBytzControlDialog()
{
    delete ui;
}

void ZBytzControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBytzControlDialog::updateList()
{
...
void ZBytzControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBytzControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZBytz_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBytzControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZBytzControlDialog::GetSelectedMints()
{
...
void ZBytzControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
