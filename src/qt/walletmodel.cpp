...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered bytz address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BYTZ.").arg(GetSporkValue(SPORK_5_SWIFTTX_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal bytz:URI (bytz:XyZ...?message=example)
            {
...
static void NotifyzBYTZReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBYTZReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBYTZReset.connect(boost::bind(NotifyzBYTZReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBYTZReset.disconnect(boost::bind(NotifyzBYTZReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zbytzTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
