...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BYTZ);
    ...
    if (!settings.contains("nAnonymizeBytzAmount"))
        settings.setValue("nAnonymizeBytzAmount", 1000);

    nAnonymizeBytzAmount = settings.value("nAnonymizeBytzAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBytzAmount"))
        SoftSetArg("-anonymizebytzamount", settings.value("nAnonymizeBytzAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bytz.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBytzAmount:
            return QVariant(nAnonymizeBytzAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBytzAmount:
            nAnonymizeBytzAmount = value.toInt();
            settings.setValue("nAnonymizeBytzAmount", nAnonymizeBytzAmount);
            emit anonymizeBytzAmountChanged(nAnonymizeBytzAmount);
            break;
        case CoinControlFeatures:
