...
enum AvailableCoinsType {
    ...
    ONLY_NONDENOMINATED_NOT350000IFMN = 4, // ONLY_NONDENOMINATED and not 350000 BYTZ at the same time
    ONLY_350000IFMN = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBYTZ send
enum ZerocoinSpendStatus {
    ZBYTZ_SPEND_OKAY = 0,                            // No error
    ZBYTZ_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBYTZ_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBYTZ_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBYTZ_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBYTZ_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBYTZ_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBYTZ_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBYTZ_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBYTZ_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBYTZ_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBYTZ_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBYTZ_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBYTZ_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBYTZ_SPENT_USED_ZBYTZ = 14,                      // Coin has already been spend
    ZBYTZ_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBYTZ_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZBytzBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZBYTZOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzbytzAuto) const;
    ...
    */
    ...
    CzBYTZWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzBYTZTracker> zbytzTracker;
    ...
    void setZWallet(CzBYTZWallet* zwallet)
    {
        zwalletMain = zwallet;
        zbytzTracker = std::unique_ptr<CzBYTZTracker>(new CzBYTZTracker(strWalletFile));
    }

    CzBYTZWallet* getZWallet() { return zwalletMain; }
    ...
    void setZBytzAutoBackups(bool fEnabled)
    {
    ...
    /** zBYTZ reset */
    boost::signals2::signal<void()> NotifyzBYTZReset;
