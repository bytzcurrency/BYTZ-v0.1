...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "bytz" is a composite category enabling all BYTZ-related debug output
            if (ptrCategory->count(string("bytz"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "bytz.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty bytz.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override bytz.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "bytzd.pid"));
