...
CzBYTZTracker::CzBYTZTracker(std::string strWalletFile)
{
...
CzBYTZTracker::~CzBYTZTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBYTZTracker::Init()
{
...
bool CzBYTZTracker::Archive(CMintMeta& meta)
{
...
bool CzBYTZTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBYTZTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBYTZTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBYTZTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBYTZTracker::GetSerialHashes()
{
...
CAmount CzBYTZTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBYTZTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBYTZTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBYTZTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBYTZTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBYTZTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBYTZTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBYTZTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBYTZTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBYTZTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBYTZTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBYTZTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBYTZTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBYTZTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBYTZTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBYTZTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBYTZTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbytz from DB\n", __func__, listDeterministicDB.size());
    }
